...
public class BinarySearchAutocomplete implements Autocompletor {
	...
	public List<Term> topMatches(String prefix, int k) {
		...
		PriorityQueue<Term> pri = new PriorityQueue<>(Comparator.comparing(Term::getWeight));
		LinkedList<Term> ret = new LinkedList<>();

		for (int i = first;i <= last ;i++){
			Term t = myTerms[i];
			if(pri.size() < k){pri.add(t);}
			else {
				if (pri.size() > 0 && pri.peek().getWeight() < t.getWeight()){
					pri.remove(); pri.add(t);
				}
			}
		}
		
		int numResults = Math.min(k, pri.size());
		
		for (int i = 0; i < numResults; i++) {
			ret.addFirst(pri.remove());
		}
		return ret;
	...
	public int sizeInBytes() {
		if (mySize == 0) {
			for(Term t : myTerms) {
			    mySize += 1*BYTES_PER_DOUBLE + BYTES_PER_CHAR*t.getWord().length();	
			}
		}
		return mySize;
	}
